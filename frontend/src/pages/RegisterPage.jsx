...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result?.message?.includes('approval')) {
    ...
    <div className="min-h-screen relative flex flex-col overflow-hidden">
      ...
      <div className="relative z-10 flex-1 flex items-start justify-center px-4 lg:px-0">
        <div className="w-full max-w-md bg-white rounded-3xl p-8 card-shadow">
          ...
          {success && (
            <div className="mb-4 p-3 rounded-xl bg-green-50 text-green-600 text-sm">
              {success}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1.5">{t.register.userType}</label>
              <input
                type="text"
                placeholder=""
                value={userType}
